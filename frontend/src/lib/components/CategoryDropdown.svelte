<script lang="ts">
	...
	import { addCustomCategory, categoryIconChoices, categoryPalette, getCategoryByName } from '$lib/stores/categories';
	...
	const selectedCategory = $derived(getCategoryByName(selected, categories));
...
<div class="relative">
	<button
		class="inline-flex min-h-11 items-center gap-3 rounded-xl border border-[#d9e4e8] bg-white px-4 py-2 text-sm font-medium text-[#173843] shadow-sm transition-colors hover:bg-[#f9fbfc]"
		type="button"
		onclick={() => (open = !open)}
	>
		{#if selectedCategory && selected}
			<span class="inline-block h-2.5 w-2.5 rounded-full" style={`background-color: ${selectedCategory.color};`}></span>
			<span class="inline-flex h-5 w-5 items-center justify-center rounded-full text-[#365661]">
				<CategoryIcon icon={selectedCategory.icon} className="h-3.5 w-3.5" />
			</span>
	...
	{#if open}
		<div class="absolute left-0 z-50 mt-2 min-w-[300px] rounded-2xl border border-[#d9e4e8] bg-white p-3 shadow-[0_22px_40px_rgba(15,23,42,0.12)]">
			<button
				class={`flex w-full items-center rounded-xl px-3 py-2 text-left text-sm font-medium transition-colors ${selected === null ? 'bg-[#f6fafb] text-[#0f5d6c]' : 'text-[#173843] hover:bg-[#f6fafb]'}`}
				type="button"
				onclick={() => choose(null)}
			>
				Tutte le categorie
